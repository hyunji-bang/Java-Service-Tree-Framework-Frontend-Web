...
$(function () {
	...
	$(function () {
		...
		var EmailList = Backbone.Collection.extend({
			...
			search: function (word, folderId) {
				...
				return this.filter(function (mail) {
					return (
						(mail.get("folderId") == folderId && pat.test(mail.get("subject"))) ||
				...
				var customWysihtml5Templates = {
					...
					lists: function (locale) {
						return (
							"<li>" +
							"<div class='btn-group'>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-command='insertUnorderedList' title='" +
							locale.lists.unordered +
							"'><i class='fa fa-list'></i></a>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-command='insertOrderedList' title='" +
							locale.lists.ordered +
							"'><i class='fa fa-th-list'></i></a>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-command='Outdent' title='" +
							locale.lists.outdent +
							"'><i class='fa fa-outdent'></i></a>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-command='Indent' title='" +
							locale.lists.indent +
					...
					link: function (locale) {
						return (
							...
							"</div>" +
							"</div>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-command='createLink' title='" +
							locale.link.insert +
					...
					image: function (locale) {
						return (
							...
							"</div>" +
							"</div>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-command='insertImage' title='" +
							locale.image.insert +
					...
					html: function (locale) {
						return (
							"<li>" +
							"<div class='btn-group'>" +
							"<a class='btn btn-sm btn-transparent ' data-wysihtml5-action='change_view' title='" +
							locale.html.edit +
		...
		var EmailListView = Backbone.View.extend({
			...
			selectRead: function () {
				this.selectNone();
				_(this.currentFolderEmails.where({ read: true })).each(function (email) {
			...
			selectUnread: function () {
				this.selectNone();
				_(this.currentFolderEmails.where({ read: false })).each(function (email) {
					email.save({ selected: true });
				});
			},
			...
			markSelectedAsRead: function () {
				_(this.currentFolderEmails.where({ selected: true })).each(function (
					email
				) {
