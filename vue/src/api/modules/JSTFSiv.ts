import { API_JSTF_SIV } from '@/constants/enums/index';
...
export default {
  getMonitor: (instance: any) => ({
    async list() {
      const { data } = await instance.get(`${API_JSTF_SIV}monitor/list.do`);
      return data;
    },
  }),
  ...
  removeNode: (instance: any) => ({
    async list(payload: changeNodePayload) {
      const { data } = await instance({
        method: 'post',
        url: `${API_JSTF_SIV}removeNode.do`,
        data: qs.stringify({
          c_id: payload.c_id,
        }),
      });
  ...
  alterNodeType: (instance: any) => ({
    async list(payload: changeNodePayload) {
      const { data } = await instance({
        method: 'post',
        url: `${API_JSTF_SIV}alterNodeType.do`,
        data: qs.stringify({
          c_id: payload.c_id,
          c_title: payload.c_title,
          c_type: payload.c_type,
